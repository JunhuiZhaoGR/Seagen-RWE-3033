...
dbGetQuery(con, glue::glue(read_file("~/RWE3033/query/diag.sql")))

options(useFancyQuotes = FALSE)

cc_codelist = codelist_load(filepath="~/RWE3033/CCI - Glasheen 2019.xlsx", sheets="Codes",no_dots=FALSE)
...
charlson <- dbGetQuery(con, glue::glue("
  WITH combined AS (
    SELECT a.patient_id
    ...
    FROM GENESIS_TEMP.PUBLIC.JZ_RWE3033_HER2DRUG AS a
    {paste0(lapply(1:n_loops, function(i) glue::glue('
      LEFT JOIN (
        SELECT DISTINCT patient_id, {sQuote(1)} AS {cc_names_short[i]}, {cc_wgts_2019[i]} AS {cc_names_short[i]}_index
        FROM GENESIS_TEMP.PUBLIC.JZ_RWE3033_diag
        WHERE  (
						(CONTAINS(DIAGNOSIS_CODE_SYSTEM,  {sQuote(9)})   AND (
							SUBSTR(diagnosis_code,1,3) IN ({icd9cm}) OR SUBSTR(diagnosis_code,1,5) IN ({icd9cm}) OR SUBSTR(diagnosis_code,1,6) IN ({icd9cm})
						))
						OR (CONTAINS(DIAGNOSIS_CODE_SYSTEM, {sQuote(10)})  AND (
							SUBSTR(diagnosis_code,1,3) IN ({icd10cm}) OR SUBSTR(diagnosis_code,1,5) IN ({icd10cm}) OR SUBSTR(diagnosis_code,1,6) IN ({icd10cm})
							))
					)
      ) AS b{i} ON a.patient_id=b{i}.patient_id
      ', icd9cm=cc_codes[[i]][['ICD-9-CM']], icd10cm=cc_codes[[i]][['ICD-10-CM']])), collapse = ' ')}
  )
  SELECT patient_id, {paste0(cc_names_short, collapse=', ')}
...
dbWriteTable(con, SQL("GENESIS_TEMP.PUBLIC.JZ_RWE3033_CCI"), charlson, overwrite = T)